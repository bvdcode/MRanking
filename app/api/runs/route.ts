...
export async function GET(request: Request) {
  try {
    ...
    const rows = (
      await getD1()
        .prepare(
          "SELECT id, pack_id, state_json, updated_at FROM runs WHERE user_id = ? ORDER BY updated_at DESC",
        )
        .bind(auth.user.id)
        .all<{
          id: string;
...
export async function PUT(request: Request) {
  try {
    ...
    const run = body.run;
    if (!run?.session?.id || !run.session.packId) {
      return Response.json({ error: "Run state required" }, { status: 400 });
    }
    const pack = await getD1()
      .prepare("SELECT owner_id FROM packs WHERE id = ? AND deleted_at IS NULL")
      .bind(run.session.packId)

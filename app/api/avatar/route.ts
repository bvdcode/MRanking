...
export async function POST(request: Request) {
  try {
    ...
    await runtimeEnv().AVATARS.put(key, file.stream(), {
    ...
    await getD1()
      ...
      .bind(key, new Date().toISOString(), auth.user.id)
      .run();
    if (previous?.avatar_key) {
      await runtimeEnv().AVATARS.delete(previous.avatar_key);
    }
    return Response.json({
      avatarUrl: `/api/avatar?user=${encodeURIComponent(auth.user.id)}&v=${Date.now()}`,
    });
  } catch (error) {
